...
class GameServiceTest {
  ...
  void makeGuess_wrongGuess_shouldKeepGameInProgress() {
    ...
    GameStateResponse state = gameService.makeGuess(code, new GuessRequest("p1", wrongGuess));
  ...
  void makeGuess_correctGuess_shouldFinishGameAndSetWinner() {
    ...
    GameStateResponse state = gameService.makeGuess(code, new GuessRequest("p1", target));
  ...
  void getState_unknownGame_shouldThrow() {
    assertThrows(IllegalArgumentException.class, () -> gameService.getState("UNKNOWN"));
  }
  ...
  void makeGuess_whenGameNotInProgress_shouldThrow() {
    ...
    assertThrows(
        IllegalStateException.class, () -> gameService.makeGuess(code, new GuessRequest("p1", 1)));
  }
}
